...
app_auth = models.application.application.Application(name='eContext Auth', description='eContext Authentication/Authorization Application')
app_auth.save()

app_api = models.application.application.Application(name='eContext API', description='eContext API for Classification')
app_auth.save()

group_api = models.group.group.Group(name='Standard API User', description="Standard users of the eContext API", customData={
    "classify_limit": 10,
    "tier_depth": 9
})
group_api.save()

group_api_admin = models.group.group.Group(name='Standard API User', description="Standard users of the eContext API", customData={
    "_no_encrypt": True,
    "company_id": 9999999,
    "tier_depth": 9999
})
